class WorldMap {
    ...
    cloneState(){
        return  {
            ...
            actorInitiative:this. state.actorInitiative,
            gameOver: this.state.gameOver,
            winner: this.state.winner,
            moves: this.state.moves,
            previousActor: this.state.previousActor,
            lastMoveIndex: this.state.lastMoveIndex
        }
    }

    actorInitiative(){
        return this.state.actorInitiative
    } 
    ...
    playMove(move){
        ...
        for (let i = this.state.height - 1; i >= 0; i--){
            ...
            if (board[index] == 0){
                board[index] = this.state.actorInitiative
                break
            }
        }
        this.state.lastMoveIndex = index
        this.state.previousActor = this.state.actorInitiative
        this.state.actorInitiative = (this.state.actorInitiative == 1) ? 2 : 1 
        this.state.moves += 1
    ...
    gameOver(){
        ...
        const previousActor = this.state.previousActor
        ...
        for (let col = startCol; col <= endCol; col++){
            ...
            if (board[index] === previousActor){
                count += 1
            } else {
                count = 0
            }
            if (count == n){
                this.state.winner = previousActor
