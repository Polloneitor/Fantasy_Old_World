                    ...
                    if(os.path.isdir(vars.custmodpth)):
                        ...
                        if not args.colab or args.savemodel:
                            ...
                            if(vars.fp32_model and ("breakmodel" not in globals() or not breakmodel.disk_blocks)):  # Use save_pretrained to convert fp32 models to fp16, unless we are using disk cache because save_pretrained is not supported in that case
                                ...
                                shutil.move(os.path.realpath(huggingface_hub.hf_hub_download(vars.model, transformers.configuration_utils.CONFIG_NAME, revision=args.revision, cache_dir="cache", local_files_only=False, legacy_cache_layout=legacy)), os.path.join("models/{}".format(vars.model.replace('/', '_')), transformers.configuration_utils.CONFIG_NAME))
                                if(utils.num_shards is None):
                                    # Save the pytorch_model.bin of an unsharded model
                                    shutil.move(os.path.realpath(huggingface_hub.hf_hub_download(vars.model, transformers.modeling_utils.WEIGHTS_NAME, revision=args.revision, cache_dir="cache", local_files_only=False, legacy_cache_layout=legacy)), os.path.join("models/{}".format(vars.model.replace('/', '_')), transformers.modeling_utils.WEIGHTS_NAME))
                                else:
...
#CareerList that comes from default + all sequence to work the program around
#CareerList = gd.gameInitCareer()
#print('Loading Game_Data')
#json_string = [ob.__dict__ for ob in CareerList]
...
def do_connect():
    ...
    #==================================================================#
    # FOW must send all data information from here.
    #==================================================================#
    #Effects
    EffectList = gd.getEffectDictionary()
    emit('from_server', {'cmd': 'geteffects', 'data': EffectList})
    #Talents
    TalentsList = gd.getTalentDictionary()
    emit('from_server', {'cmd': 'gettalents', 'data': TalentsList})

    #Skills
    SkillsList = gd.getSkillDictionary()
    emit('from_server', {'cmd': 'getskills', 'data': SkillsList})

    #Career 
    CareerList = gd.getCareerDictionary()
    emit('from_server', {'cmd': 'getcareer', 'data': CareerList})

    #Items
    Items = gd.getWeaponDictionary()
    emit('from_server', {'cmd': 'getitems', 'data': Items})

    #Player
    demo_player = gd.gameInitDemoPlayer()

    fow_demopath = "FantasyOldWorldData/player/actor/"
    #Player Data Sent (DEMO)
    out_file = open(fow_demopath+demo_player.__dict__['actor_name']+".json", "w")

    json.dump(demo_player.__dict__, out_file, indent = 6)

    out_file.close()

    emit('from_server', {'cmd': 'demo_player', 'data': demo_player.__dict__})
...
def get_message(msg):
    ...
    if(msg['cmd'] == 'submit'):
        if(vars.mode == "play"):
            ...
            memsubmit(msg['data'])
    # Fantasy Old World Websockets cases
    elif(msg['cmd'] == 'request_demo_player'):
        demo_player = gd.gameInitDemoPlayer()
        data = demo_player.__dict__
        #"print(demo_player)
        #"print(data)
        emit('from_server', {'cmd': 'demo_player', 'data': data})
    # Retry Action
    elif(msg['cmd'] == 'retry'):
        if(vars.aibusy):
            if(msg.get('allowabort', False)):
...
def inlinedelete(chunk):
    ...
    send_debug()
#==================================================================#
#   FOW: Updates the client with the information that the user must be granted based on what's current
#==================================================================#
